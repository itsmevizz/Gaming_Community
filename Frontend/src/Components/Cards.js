...
function CommunityCard(props) {
  const myGroup = props.myGroup
  const membersP = props.members
  const Navigate = useNavigate()
  const { communities, loading } = useSelector((state) => ({ ...state.communities }))
  const { members } = useSelector((state) => (state.members))
  const loading1 = useSelector((state) => (state.members.loading))
  ...
  return (
    <div className={``}>
      {loading ? <div className='mt-[25%] grid place-content-center'>
        ...
        <div className="flex flex-wrap m-3 ">
          {img.map((url, index) => {
            return (
              <motion.div className={`w-full sm:w-1/2  ${props.members ? "md:w-1/6" : "md:w-1/4"} flex flex-col p-3 `}
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
              >
