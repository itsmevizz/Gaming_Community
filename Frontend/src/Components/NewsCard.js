...
function NewsCard(props) {
  ...
  return (
    <div
      class={`${props.value} left-[110px] lg:left-72 w-[65%] sm:w-[70%] md:w-[75%] lg:w-[80%] lg:my-[20px] duration-500 relative select-none`}
    >
      {loading ? (
          ...
          {news.map((content, key) => {
            if (content.urlToImage) {
              return (
                <motion.div
                  key={key}
                  class="grid overflow-hidden shadow-md hover:shadow-2xl bg-white dark:bg-[#1a2634] rounded-lg duration-300 hover:cursor-pointer"
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                >
                  <div
                    class={`${
                      content.urlToImage ? "min-h-[150px]" : " min-h-0"
                    } rounded-r-full lg:rounded-full flex-none bg-cover bg-right rounded-t lg:rounded-t-none lg:rounded-l text-center overflow-hidden`}
                  ...
                  <div class=" flex flex-col justify-between lg:rounded-r p-4 ">
                    ...
                    <div class="items-center mt-4 ">
                      ...
                      <p class="text-gray-600 mt-1 dark:text-gray-200 text-xs">{`${new Date(
                        content.publishedAt
        ...
        <div className="mt-[25%] grid place-content-center">
          <div class=" spinner-border animate-spin inline-block w-10 h-10 border-4 rounded-full text-red-600 ml-5 mb-2"></div>
